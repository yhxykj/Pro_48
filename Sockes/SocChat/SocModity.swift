...
enum SocModity {
    ...
    private struct SocDetailSign {
var privacy_margin: Double? = 0
var homePadding: Float? = 0
var edit_padding: Double? = 0
    ...
    static func reply(for message: String, historyCount: Int) -> String {
       var connectingX: [Any]! = [String(cString: [110,111,110,110,117,108,108,111,117,116,0], encoding: .utf8)!]
      connectingX.append(3 & connectingX.count)

        let perla = message.lowercased()
        let template = templates.first { template in
            template.keywords.contains { keyword in
                perla.contains(keyword.lowercased())
            }
        } ?? fallbackTemplate
    ...
    private static func selectReply(
       ...
       var p_productsc: Float = 5.0
      p_productsc += (Float(Int(p_productsc > 285313569.0 || p_productsc < -285313569.0 ? 77.0 : p_productsc)))

        guard !replies.isEmpty else { return "" }

        let test = message.unicodeScalars.reduce(historyCount) { result, scalar in
            result &+ Int(scalar.value)
        }
        let bundle = Int(test.magnitude % UInt(replies.count))
        return replies[bundle]
    }

    private static let templates: [SocDetailSign] = [
        SocDetailSign(
            keywords: [
    ...
    private static let fallbackTemplate = SocDetailSign(
        keywords: [],
        replies: [
